...
class RemoBarra (private val datosEjercicio: DatosEjercicioActual): TipoDeEjercicio {
    val ayudaTrigonometrica = AyudaTrigonometrica()
    val imageResources = listOf(
        R.drawable.remobarra
    )
    ...
    override fun analyzeDataEjercicio(
        ...
        if(  confirmAngle(angleA,angles[datosEjercicio.form][0]) > probabilidad &&
            ...
            confirmAngle(angleI,angles[datosEjercicio.form][8]) > probabilidad &&
            confirmAngle(angleJ,angles[datosEjercicio.form][9]) > probabilidad
        ){
    ...
    override fun getPoseCSV(): List<String> {
        return listOf("pose/fitness_poses_barras_out.csv","br_down")
    }
