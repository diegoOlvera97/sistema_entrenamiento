...
class LegRaises (private val datosEjercicio: DatosEjercicioActual): TipoDeEjercicio {
    ...
    override fun analyzeDataEjercicio(
        ...
        datosEjercicio.porcentajeSucces = ((
                ...
                abs((angleF-angles[datosEjercicio.form][5])/(angles[datosEjercicio.form][5])))/2)*100

        if( confirmAngle(angleD,angles[datosEjercicio.form][3]) > probabilidad &&
    ...
    override fun getPoseCSV(): List<String> {
        return listOf("pose/fitness_poses_leg_out.csv","leg_down")
    }
